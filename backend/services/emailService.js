...
const sendMeetingNotification = async (user, meeting) => {
  ...
  const row1 = '<tr style="background:#f9fafb"><td style="padding:12px;font-weight:bold;color:#6b7280;font-size:12px;width:35%">MEETING</td><td style="padding:12px;font-size:14px;font-weight:600">' + meeting.title + '</td></tr>';
  const row2 = '<tr><td style="padding:12px;font-weight:bold;color:#6b7280;font-size:12px">DATE AND TIME</td><td style="padding:12px;font-size:14px">' + meetingDate + '</td></tr>';
  const row3 = '<tr style="background:#f9fafb"><td style="padding:12px;font-weight:bold;color:#6b7280;font-size:12px">VENUE</td><td style="padding:12px;font-size:14px">' + meeting.venue + '</td></tr>';
  const row4 = '<tr><td style="padding:12px;font-weight:bold;color:#6b7280;font-size:12px">TOPICS</td><td style="padding:12px;font-size:14px">' + (meeting.topics ? meeting.topics.join(', ') : 'NA') + '</td></tr>';

  const html = '<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;border:1px solid #e5e7eb;border-radius:12px;overflow:hidden">'
    + '<div style="background:#166534;padding:24px 32px">'
    + '<h1 style="color:#ffffff;margin:0;font-size:22px">GramMeet</h1>'
    + '<p style="color:#bbf7d0;margin:4px 0 0;font-size:13px">Gram Panchayat Meeting System</p>'
    + '</div>'
...
const sendReminder = async (user, meeting, type) => {
  ...
  const row1 = '<tr style="background:#f9fafb"><td style="padding:12px;font-weight:bold;color:#6b7280;font-size:12px;width:35%">MEETING</td><td style="padding:12px;font-size:14px;font-weight:600">' + meeting.title + '</td></tr>';
  const row2 = '<tr><td style="padding:12px;font-weight:bold;color:#6b7280;font-size:12px">DATE AND TIME</td><td style="padding:12px;font-size:14px"><strong>' + meetingDate + '</strong></td></tr>';
  const row3 = '<tr style="background:#f9fafb"><td style="padding:12px;font-weight:bold;color:#6b7280;font-size:12px">VENUE</td><td style="padding:12px;font-size:14px">' + meeting.venue + '</td></tr>';

  const html = '<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;border:1px solid #e5e7eb;border-radius:12px;overflow:hidden">'
    + '<div style="background:' + bgColor + ';padding:24px 32px">'
    + '<h1 style="color:#ffffff;margin:0;font-size:22px">' + headingText + '</h1>'
    + '<p style="color:#ffffff;margin:4px 0 0;font-size:13px;opacity:0.8">GramMeet Notification</p>'
    + '</div>'
    + '<div style="padding:32px">'
    + '<p style="color:#374151;font-size:15px">Dear <strong>' + user.name + '</strong>,</p>'
    + '<p style="color:#374151;font-size:14px">This is a reminder. Your meeting starts in <strong>' + timeLabel + '</strong>.</p>'
    + '<table style="width:100%;border-collapse:collapse;margin:20px 0">'
    + row1 + row2 + row3
    + '</table>'
    + '<p style="color:#6b7280;font-size:13px;margin-top:24px">Regards,<br><strong style="color:#166534">GramMeet System</strong></p>'
